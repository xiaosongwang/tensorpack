...
from ..utils.stats import RatioCounter, BinaryStatistics, MultiLabelStatistics
from ..tfutils import get_op_tensor_name

__all__ = ['ScalarStats', 'Inferencer',
           'ClassificationError', 'BinaryClassificationStats', 'MultiLabelClassificationStats']
...
class MultiLabelClassificationError(Inferencer):
    """
    Compute classification error in batch mode, from a ``wrong`` tensor.
...
class MultiLabelClassificationStats(Inferencer):
    """
    Compute precision / recall in binary classification, given the
    prediction vector and the label vector.
    """
    ...
    def _before_inference(self):
        self.stat = MultiLabelStatistics()
    ...
    def _after_inference(self):
        return {self.prefix + '_precision': self.stat.precision,
                self.prefix + '_recall': self.stat.recall,
                self.prefix + '_specificity': self.stat.specificity}
